<x-app-layout title="Monitoring Bobot Penilai 360 (Admin RS)">
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <h1 class="text-2xl font-semibold text-slate-800">Monitoring Bobot Penilai 360</h1>
        </div>
    </x-slot>
    ...
    <div class="container-px py-6 space-y-6">
        ...
        <form method="GET" class="bg-white rounded-2xl shadow-sm p-6 border border-slate-100">
            ...
            <div class="mt-6 flex justify-end gap-3">
                <a href="{{ route('admin_rs.unit_rater_weights.units') }}" class="inline-flex items-center gap-2 h-12 px-6 rounded-xl text-[15px] font-medium text-slate-600 bg-white border border-slate-200 hover:bg-slate-50">
            ...
            @forelse($units as $u)
                <tr class="hover:bg-slate-50">
                    ...
                    <td class="px-6 py-4 text-right">
                        <x-ui.button as="a" href="{{ route('admin_rs.unit_rater_weights.unit', $u->id) }}" variant="success" class="h-9 px-3 text-sm">Detail</x-ui.button>
                    </td>
                </tr>
            @empty
