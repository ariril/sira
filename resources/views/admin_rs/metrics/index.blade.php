    ...
    <div class="container-px py-6 space-y-6">
        @unless($latestImportablePeriod ?? null)
            <div class="rounded-xl border border-rose-200 bg-rose-50 text-rose-800 px-4 py-3 text-sm">
                ...
                <div>Import metrics (rekap bulanan) hanya dapat dilakukan ketika periode sudah dikunci (LOCKED) atau sedang revisi (REVISION).</div>
            </div>
        @endunless
        ...
        @if($latestImportablePeriod ?? null)
            <div class="bg-white rounded-2xl shadow-sm border border-slate-100 p-6 space-y-4">
                ...
                <form method="POST" action="{{ route('admin_rs.metrics.template') }}" class="space-y-4">
                    ...
                    <input type="hidden" name="period_id" value="{{ (int) ($latestImportablePeriod->id ?? 0) }}" />
