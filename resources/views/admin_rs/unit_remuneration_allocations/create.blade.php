<x-app-layout title="Tambah Alokasi Unit">
    <x-slot name="header">
        <div class="flex items-center justify-between">
            ...
            <x-ui.button as="a" href="{{ route('admin_rs.unit-remuneration-allocations.index') }}"
                variant="outline" class="h-12 px-6 text-base">Kembali</x-ui.button>
        </div>
    </x-slot>
        ...
        <div class="bg-white rounded-2xl shadow-sm p-6 border border-slate-100">
            <form method="POST" action="{{ route('admin_rs.unit-remuneration-allocations.store') }}" class="space-y-6"
                ...
                <div class="space-y-4">
                    <div class="grid md:grid-cols-3 gap-4">
                        <label class="space-y-1">
                            ...
                            <x-ui.select name="assessment_period_id" :options="$periods->pluck('name', 'id')" placeholder="Pilih periode" class="h-11" />
                        </label>
                        <label class="space-y-1">
                            <span class="text-sm font-medium text-slate-700">Unit</span>
                            <x-ui.select name="unit_id" :options="$units->pluck('name', 'id')" placeholder="Pilih unit" id="unit-select" class="h-11" />
                        </label>
                        <div class="space-y-1">
                            <span class="text-sm font-medium text-slate-700">Total Alokasi</span>
                            <div
                                class="h-11 flex items-center px-3 rounded-lg border border-slate-200 text-sm text-slate-700 bg-slate-50">
                                <span id="amount-hint" class="font-semibold">Rp 0</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <x-ui.textarea name="note" rows="3" value="{{ old('note', $item->note) }}"
                ...
                <div class="flex justify-end gap-3">
                    <x-ui.button as="a" href="{{ route('admin_rs.unit-remuneration-allocations.index') }}"
                        variant="outline">Batal</x-ui.button>
    ...
    <script>
        (() => {
            ...
            // Initial render if unit already selected
            if (unitSelect?.value) {
