                ...
                <div class="space-y-4">
                    <div class="grid md:grid-cols-3 gap-4">
                        <label class="space-y-1">
                            ...
                            <x-ui.select name="assessment_period_id" :options="$periods->pluck('name','id')" :value="old('assessment_period_id', $item->assessment_period_id)" placeholder="Pilih periode" class="h-11" />
                        </label>
                        <label class="space-y-1">
                            <span class="text-sm font-medium text-slate-700">Unit</span>
                            <x-ui.select name="unit_id" :options="$units->pluck('name','id')" :value="old('unit_id', $item->unit_id)" placeholder="Pilih unit" id="unit-select" class="h-11" />
                        </label>
                        <div class="space-y-1">
    ...
    (() => {
        ...
        const renderRows = (unitId) => {
            ...
            profIds.forEach((pid) => {
                ...
                wrapper.className = 'flex items-center justify-between gap-3 rounded-lg border border-slate-200 px-3 py-2';
                wrapper.innerHTML = `
                    <div class="space-y-1">
