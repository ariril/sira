<x-app-layout title="Detail Bobot Kriteria Unit (Admin RS)">
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <h1 class="text-2xl font-semibold text-slate-800">Detail Bobot Kriteria - {{ $unit->name ?? '-' }}</h1>
            <x-ui.button as="a" href="{{ route('admin_rs.unit-criteria-weights.index') }}" variant="success" class="h-12 px-6 text-base">Kembali</x-ui.button>
        </div>
    </x-slot>
