...
@section('content')
    <div class="max-w-7xl mx-auto px-4 sm:px-6 py-10">
        ...
        <div class="flex flex-col md:flex-row md:items-end md:justify-between gap-6 mb-10">
            ...
            <form method="GET" action="{{ route('announcements.index') }}" class="w-full md:w-[420px]">
        ...
        <div class="flex flex-wrap gap-2 mb-8">
            ...
            @foreach (['remunerasi','kinerja','panduan','lainnya'] as $k)
                     <a href="{{ route('announcements.index', array_filter(['kategori'=>$k,'label'=>$activeLbl,'q'=>request('q')])) }}"
            ...
            @foreach (['penting','info','update'] as $l)
                     <a href="{{ route('announcements.index', array_filter(['label'=>$l,'kategori'=>$activeCat,'q'=>request('q')])) }}"
        ...
        <div class="grid gap-8 md:grid-cols-2 xl:grid-cols-3">
                        @forelse ($items as $i => $p)
                                @php
                                        $label = $p->label?->value; // enum -> string
                                        $badge = match($label){
                                        ...
                                        $isFeatured = $i === 0 || (bool) $p->is_featured;
                                @endphp
                ...
                <article class="bg-white rounded-xl p-6 shadow-md hover:shadow-lg hover:-translate-y-1 transition border border-slate-100 {{ $isFeatured ? 'outline outline-2 outline-red-500/20' : '' }}">
                    <div class="flex items-center justify-between mb-3">
                                <span class="px-3 py-1 rounded-full text-xs font-semibold {{ $badge['bg'] }} {{ $badge['text'] }} uppercase">
                                    {{ ucfirst($p->label?->value ?? 'info') }}
          </span>
                                                <time class="text-slate-500">{{ optional($p->published_at)->translatedFormat('d F Y') }}</time>
                    </div>

                    <h2 class="text-xl font-semibold text-slate-800 mb-2">
                                                <a href="{{ route('announcements.show', $p->slug) }}" class="hover:underline">{{ $p->title }}</a>
                    </h2>

                    <p class="text-slate-600">
                                                {{ $p->summary ? \Illuminate\Support\Str::limit(strip_tags($p->summary), 150)
