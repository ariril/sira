<x-app-layout title="Detail Bobot Penilai 360 — Kepala Poliklinik">
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <h1 class="text-2xl font-semibold text-slate-800">Detail Bobot Penilai 360 — {{ $unit->name ?? '-' }}</h1>
            <x-ui.button as="a" href="{{ route('kepala_poliklinik.unit_rater_weights.units') }}" variant="violet" class="h-12 px-6 text-base">Kembali</x-ui.button>
        </div>
    </x-slot>
        ...
        <x-ui.table min-width="1100px">
            <x-slot name="head">
                <tr>
                    ...
                    <th class="px-6 py-4 text-left">Profesi Dinilai</th>
            ...
            @forelse($items as $it)
                @php
                    $st = (string) ($it->status ?? 'draft');
                    $assessorType = (string) ($it->assessor_type ?? '');
                    $assessorLevel = $it->assessor_level;
                    $assessorProfession = trim((string) ($it->assessor_profession_name ?? ''));
                    $assessorLabel = match ($assessorType) {
                        'self' => 'Diri sendiri',
                        'supervisor' => 'Atasan',
                        'peer' => 'Rekan',
                        'subordinate' => 'Bawahan',
                        default => $assessorType !== '' ? $assessorType : '-',
                    };
                    if ($assessorProfession !== '') {
                        $assessorLabel .= ' - ' . $assessorProfession;
                    }
                @endphp
                <tr class="hover:bg-slate-50">
                    <td class="px-6 py-4">{{ $it->criteria_name ?? '-' }}</td>
                    <td class="px-6 py-4">{{ $it->assessee_profession_name ?? '-' }}</td>
                    <td class="px-6 py-4">{{ $assessorLabel }}</td>
                    <td class="px-6 py-4">{{ $assessorLevel ? 'L' . $assessorLevel : '-' }}</td>
