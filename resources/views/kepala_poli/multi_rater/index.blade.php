                ...
                <x-ui.table min-width="840px">
                    ...
                    @forelse($savedScores as $s)
                        <tr class="hover:bg-slate-50">
                            ...
                            <td class="px-6 py-3">
                                @if($allowInlineEdit)
                                    <form class="inline-flex items-center gap-2" onsubmit="event.preventDefault(); const fd=new FormData(this); fetch('{{ route('kepala_poliklinik.multi_rater.store') }}',{method:'POST',headers:{'X-CSRF-TOKEN':'{{ csrf_token() }}','Accept':'application/json'},body:fd}).then(r=>r.json()).then(()=>location.reload());">
