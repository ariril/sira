<x-app-layout title="Isi Penilaian 360">
    ...
    <div class="container-px py-6">
        ...
        @endif
        <form method="POST" action="{{ route('kepala_poliklinik.multi_rater.submit', $assessment) }}" class="space-y-4">
            @csrf
            ...
            <div>
                <button class="px-4 py-2 bg-purple-600 text-white rounded disabled:opacity-60" type="submit" @disabled(!($canSubmit ?? false))>Kirim</button>
            </div>
        </form>
    </div>
</x-app-layout>
