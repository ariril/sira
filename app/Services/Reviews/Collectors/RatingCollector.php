...
namespace App\Services\Reviews\Collectors;
...
class RatingCollector implements CriteriaCollector
{
    ...
    public function collect(AssessmentPeriod $period, int $unitId, array $userIds): array
    {
        ...
        $startDate = $start ? Carbon::parse((string) $start)->toDateString() : null;
        $endDate = $end ? Carbon::parse((string) $end)->toDateString() : null;
        ...
        if ($startDate) {
            $query->whereDate('reviews.decided_at', '>=', $startDate);
        }
        if ($endDate) {
            $query->whereDate('reviews.decided_at', '<=', $endDate);
        }
    ...
    public function readiness(AssessmentPeriod $period, int $unitId): array
    {
        ...
        if ($startDate) {
            $q->whereDate('decided_at', '>=', $startDate);
        }
        if ($endDate) {
            $q->whereDate('decided_at', '<=', $endDate);
        }
