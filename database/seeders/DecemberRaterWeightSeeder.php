...
class DecemberRaterWeightSeeder extends Seeder
{
    public function run(): void
    {
        ...
        if (empty($periodIds)) {
            $this->command?->warn('DecemberRaterWeightSeeder: target assessment period not found. Set env RATER_WEIGHT_PERIOD_ID to force a period.');
            return;
        }
        ...
        foreach ($periodIds as $periodId) {
            ...
            if (!$period) {
                $this->command?->warn("DecemberRaterWeightSeeder: period_id={$periodId} not found.");
                continue;
            }
            ...
            if (empty($unitIds)) {
                $this->command?->warn("DecemberRaterWeightSeeder: no unit_criteria_weights found for period_id={$periodId}.");
                continue;
            }
            ...
            DB::transaction(function () use ($groups, &$updated) {
                foreach ($groups as $groupRows) {
                    ...
                    // Non-destructive rule: only fill when ALL weights are still null (or all zeros).
